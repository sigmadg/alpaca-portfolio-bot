...
CONFIG = {
    ...
    'mini_monitor_interval_seconds': 60,   # Notificaciones (mini reporte) cada 60 s, igual que alto riesgo
    ...
    'etiqueta_cartera': 'BAJO RIESGO',
}
# Merge config desde helpers (risk, cooldown, max_drawdown, max_trades, mini_monitor). Fuente de verdad para reporte grande: full_report_interval_minutes.
...
def sistema_completo_alpaca(
    ...
    if CONFIG.get('usar_top_5_acciones'):
        ...
        if precios_actuales:
            pesos_optimizados = trading.identificar_top_acciones(
                ...
                criterio=CONFIG.get('top_5_criterio', 'ganancia_pct'),
    ...
    logger.info("[LOW_RISK] Cartera (largo plazo): %d activos (top %s por ganancia)", len(pesos_optimizados), CONFIG.get('top_5_numero', 4))
    ...
    # Risk checks antes de colocar órdenes (drawdown, max_trades)
    peak_equity = None
    orders_in_window = []
    ...
    if trading.api:
        try:
            ...
            peak_equity = float(acc.portfolio_value)
        except Exception:
            pass
    ...
    if monitorear:
        ...
        try:
            while True:
                ...
                # Actualizar peak (con safe_api si existe)
                try:
                    if trading.api:
                ...
                if debe_full:
                    ...
                    try:
                        ...
                        if resultados_refresh:
                            ...
                            rm = resultados_refresh.get('rendimientos_anuales') or resultados_refresh.get('rendimientos_mensuales')
                            ...
                            if total_p > 0 and nuevos_pesos:
                                ...
                                logger.info("[LOW_RISK] Top 4 actualizado para reporte: %s", list(pesos_optimizados.keys()))
                    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        CONFIG['top_5_numero'] = 4
        CONFIG['top_5_criterio'] = 'ganancia_pct'

        riesgo_max = CONFIG.get('riesgo_max_anual', CONFIG['riesgo_max'])
        print(f"\n🚀 Iniciando cartera: largo_plazo | riesgo_max ANUAL: {riesgo_max*100:.1f}%")
        print("   📌 Solo las 4 mejores (ganancia positiva; se venden las que no están en el top)")
        print("=" * 80)
        ...
        resultado = sistema_completo_alpaca(
            ...
            tickers_completos=None,
            kwargs_analisis={'estrategia': 'sharpe', 'generar_graficos': False},
            analizar_cartera_largo_plazo=None,
        )
